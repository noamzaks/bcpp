...
template <typename T> void HashTable<T>::initializeFrom(const HashTable<T>& other) {
...
template <typename T> HashTable<T>::HashTable(const HashTable<T>& other) {
...
template <typename T> T& HashTable<T>::operator[](const std::string& key) {
    auto& item = get(key);
    if (item == nullptr) {
...
template <typename T> void HashTable<T>::insert(const std::string& key, const T& value) {
    auto& item = get(key);
    item = std::make_unique<std::pair<std::string, T>>(std::make_pair(key, value));
}

template <typename T> void HashTable<T>::pop(const std::string& key) {
    auto& item = get(key);
    item = nullptr;
}

template <typename T> size_t HashTable<T>::size() {
    size_t result = 0;
    for (size_t i = 0; i < HASH_TABLE_SIZE; i++) {
...
template <typename T> std::string HashTable<T>::find(const T& item) {
    for (size_t i = 0; i < HASH_TABLE_SIZE; i++) {
...
template <typename T> bool HashTable<T>::has(const std::string& key) {
    auto& item = get(key);
    return item != nullptr;
}

template <typename T> std::unique_ptr<std::pair<std::string, T>>& HashTable<T>::get(const std::string& key) {
